...
import tech.icrossing.lsm.voiceVideoCalls.CallsActivityNoService;
import tech.icrossing.lsm.websocket_connection.HLSocketConnection;
import tech.icrossing.lsm.websocket_connection.HLWebSocketAdapter;
import tech.icrossing.lsm.websocket_connection.HLWebSocketClient;
import tech.icrossing.lsm.websocket_connection.OnConnectionChangedListener;
...
//		}
	...
	protected void onStart() {
		...
		HLSocketConnection connection = LUISSApp.getSocketConnection();
		if (connection != null) {
			...
			public void run() {
				if (rootContent != null) {
					...
					try {
						...
						Utils.applyFontToTextView(textView, R.string.osSemiBold);
						Utils.applyFontToTextView(actionView, R.string.osBold);
					} catch (Exception e) {
						e.printStackTrace();
					}
				} else {
