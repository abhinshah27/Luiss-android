...
import tech.icrossing.lsm.models.HLInterests;
...
import tech.icrossing.lsm.voiceVideoCalls.SDKTokenUtils;
...
//		onRestoreInstanceState(savedInstanceState != null ? savedInstanceState : getArguments());
	...
	public void onPause() {
		...
		LUISSApp.notificationsFragmentVisible = false;
...
//		if (context instanceof OnNotificationsClickListener) {
//			mNotifClickListener = (OnNotificationsClickListener) context;
//		} else {
//			throw new RuntimeException(context.toString()
//					+ " must implement OnNotificationsClickListener");
//		}
	}
...
//								mUser = RealmUtils.checkAndFetchUser(realm, mUser);
	...
	public void onDotsClick(View dots, final View preferredLabel) {
		if (profileType == ProfileHelper.ProfileType.INTEREST_CLAIMED ||
			...
			Utils.applyFontToMenuItem(getContext(), prefer, R.string.osReg);
			Utils.applyFontToMenuItem(getContext(), unprefer, R.string.osReg);
			Utils.applyFontToMenuItem(getContext(), claim, R.string.osReg);
	...
	public void onDotsClickUser(View dots) {
		if (profileType == ProfileHelper.ProfileType.FRIEND ||
			...
			Utils.applyFontToMenuItem(getContext(), remove, R.string.osReg);
			Utils.applyFontToMenuItem(getContext(), block, R.string.osReg);
			Utils.applyFontToMenuItem(getContext(), report, R.string.osReg);
	...
	private void flagUser(final FlagType type) {
		...
		if (type != null) {
			...
			if (moderationDialog != null && userGeneric != null) {
				...
				if (v != null) {
					...
					negativeBtn.setOnClickListener(view -> moderationDialog.dismiss());
				}
