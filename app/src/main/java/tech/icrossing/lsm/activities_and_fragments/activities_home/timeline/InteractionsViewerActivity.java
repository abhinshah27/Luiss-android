...
package tech.icrossing.lsm.activities_and_fragments.activities_home.timeline;
...
import tech.icrossing.lsm.adapters.InteractionCommentsAdapter;
import tech.icrossing.lsm.adapters.InteractionHeartsAdapter;
import tech.icrossing.lsm.adapters.InteractionSharesAdapter;
...
import tech.icrossing.lsm.utility.LoadMoreResponseHandlerTask;
import tech.icrossing.lsm.utility.LoadMoreScrollListener;
import tech.icrossing.lsm.utility.LogUtils;
import tech.icrossing.lsm.utility.SlidingPanelHandler;
import tech.icrossing.lsm.utility.Utils;
import tech.icrossing.lsm.utility.helpers.RealTimeCommunicationHelperKotlin;
import tech.icrossing.lsm.utility.helpers.RealTimeCommunicationListener;
...
public class InteractionsViewerActivity extends HLActivity implements View.OnClickListener,
	...
	public int getLastPageId() {
		if (hUp)
			return LUISSApp.pageIdHearts;
		else if (cUp)
			return LUISSApp.pageIdComments;
		else if (sUp)
			return LUISSApp.pageIdShares;
...
//			cntCommentsPost.setTextColor(Utils.getColor(this, colorRes));
	...
	private void callForLists(@NonNull HLServerCalls.ListsCallType type, @NonNull String listName) {
		...
		HLRequestTracker.getInstance(((LUISSApp) getApplication())).handleCallResult(this, this, result);
	}
					...
					if (heartsList != null) {
						...
						if (!background) {
							heartsAdapter.notifyItemRangeInserted(
									(Constants.PAGINATION_AMOUNT * (LUISSApp.pageIdHearts - 1)), newItemsCount
							);
						}
					}
				}
...
//							commentsList.addAll(HLPosts.getInstance().getVisibleCommentsForPost(mPostId));
						if (!background) {
							commentsAdapter.notifyItemRangeInserted(
									(Constants.PAGINATION_AMOUNT * (LUISSApp.pageIdComments) - 1), newItemsCount
							);
						}
					}
				}
					...
					if (sharesList != null) {
						...
						if (!background) {
							sharesAdapter.notifyItemRangeInserted(
									(Constants.PAGINATION_AMOUNT * (LUISSApp.pageIdShares - 1)), newItemsCount
							);
						}
					}
				}
...
//				cntSharesPost.setSelected(mPost.isYouDidShares());
	...
	private void call(boolean usePageId) {
		...
		switch (viewHitResId) {
			case R.id.button_hearts:
				type = InteractionPost.Type.HEARTS;
				if (usePageId) pageId = ++LUISSApp.pageIdHearts;
				else pageId = -1;
				break;

			case R.id.button_comments:
				type = InteractionPost.Type.COMMENT;
				if (usePageId) pageId = ++LUISSApp.pageIdComments;
				else pageId = -1;
				break;

			case R.id.button_shares:
				type = InteractionPost.Type.SHARE;
				if (usePageId) pageId = ++LUISSApp.pageIdShares;
				else pageId = -1;
				break;
		}
...
//						hUp = false;
					...
					break;
				case R.id.button_comments:
					first = (LUISSApp.pageIdComments == 1);
//					if (mPost != null)
