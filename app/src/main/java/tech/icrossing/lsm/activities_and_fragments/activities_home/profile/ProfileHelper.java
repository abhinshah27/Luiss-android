...
import tech.icrossing.lsm.activities_and_fragments.activities_chat.ChatActivity;
	...
	private void setLayoutForUsers(@NonNull ProfileType type, @Nullable HLUserGeneric userCard) {
		...
		if (type == ProfileType.ME) {
			...
			inviteBtn.setVisibility(
					(
							!LUISSApp.getSocketConnection().isConnected() || type == ProfileType.FRIEND ||
			...
			if (documentationBtn != null)
				documentationBtn.setVisibility(View.GONE);
		}
...
//  					mainBackground.setImageDrawable(d);
	...
	private void callServer() {
		...
		HLRequestTracker.getInstance(((LUISSApp) mListener.getProfileFragment().getActivity().getApplication()))
