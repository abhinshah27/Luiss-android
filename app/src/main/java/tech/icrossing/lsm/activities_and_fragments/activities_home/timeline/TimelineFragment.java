...
import tech.icrossing.lsm.activities_and_fragments.activities_home.OnActionsResultFromBottom;
import tech.icrossing.lsm.activities_and_fragments.activities_home.global_search.GlobalSearchActivity;
import tech.icrossing.lsm.activities_and_fragments.activities_home.profile.ProfileActivity;
import tech.icrossing.lsm.activities_and_fragments.activities_home.settings.SettingsActivity;
import tech.icrossing.lsm.adapters.FeedMemoryViewHolder;
import tech.icrossing.lsm.adapters.InnerCircleFeedAdapter;
...
import tech.icrossing.lsm.models.FeedFilter;
...
import tech.icrossing.lsm.models.enums.FeedFilterTypeEnum;
...
//			((HomeActivity) getActivity()).setPostDeletedListener(this);
	...
	public void onResume() {
		...
		if (usageType == FragmentUsageType.TIMELINE) {
			if (/*LUISSApp.refreshAllowed*/false) {
...
//				getContext().unregisterReceiver(filterRcvr);
	...
	private void onRestoreInstanceState(Bundle arguments, Bundle savedInstanceState) {
		...
		if (filtersString != null && !filtersString.isEmpty()) {
			LUISSApp.refreshAllowed = !filtersString.containsAll(mUser.getSelectedFeedFilters()) ||
					!mUser.getSelectedFeedFilters().containsAll(filtersString);

			if (LUISSApp.refreshAllowed)
...
//				LocalBroadcastManager.getInstance(getContext()).unregisterReceiver(realTimeHelper.getServerMessageReceiver());
	...
	private void callCircles() {
		...
		HLRequestTracker.getInstance(((LUISSApp) getActivity().getApplication()))
