...
package tech.icrossing.lsm.activities_and_fragments.activities_create_post
...
import tech.icrossing.lsm.models.enums.*
import tech.icrossing.lsm.utility.*
import tech.icrossing.lsm.utility.helpers.HLMediaType
import tech.icrossing.lsm.utility.helpers.LIMIT_MIN_SIZE_FOR_UPLOAD_VIDEO
import tech.icrossing.lsm.utility.helpers.MediaHelper
import tech.icrossing.lsm.utility.helpers.MediaUploadManager
...
class CreatePostPreviewActivity: HLActivity(), OnServerMessageReceivedListener, OnMissingConnectionListener,
    ...
    private fun initLayout() {
        ...
        if (mediaCaptureType == HLMediaType.AUDIO) {
            if (LUISSApp.siriComposition != null)
                waveAnimation.setComposition(LUISSApp.siriComposition)
            else {
                LUISSApp.initLottieAnimation(this, object : LottieCompositioListener() {
    ...
    private fun attemptSendingPost(fromWiFiDialog: Boolean) {
        ...
        if (!checkConnectionAndShow(fromWiFiDialog)) {
            ...
            HLRequestTracker.getInstance(application as LUISSApp).handleCallResult(this, this, results)
        }
    }
