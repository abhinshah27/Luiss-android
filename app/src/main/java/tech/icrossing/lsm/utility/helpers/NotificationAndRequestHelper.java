...
import tech.icrossing.lsm.activities_and_fragments.activities_home.profile.SinglePostActivity;
import tech.icrossing.lsm.activities_and_fragments.activities_home.profile.user_documentation.LuissDocumentationActivity1List;
import tech.icrossing.lsm.adapters.NotificationsAdapter;
...
import tech.icrossing.lsm.models.enums.NotificationNatureEnum;
import tech.icrossing.lsm.models.enums.NotificationTypeEnum;
import tech.icrossing.lsm.models.enums.RequestsStatusEnum;
...
//				}
		...
		if (background) {
			activity.runOnUiThread(() -> {
				if (!LUISSApp.notificationsFragmentVisible)
	...
	public void handleDotVisibility(Boolean show) {
		if (mListener.getBottomBarNotificationDot() != null) {
			...
			mListener.getBottomBarNotificationDot().setVisibility(
					show && activity.getUser().isValid() && !LUISSApp.notificationsFragmentVisible ?
	...
	public static void handleDotVisibility(View dot, boolean validUser) {
		if (dot != null) {
			dot.setVisibility((HLNotifications.getInstance().getUnreadCount(true) > 0  && !LUISSApp.notificationsFragmentVisible && validUser) ? View.VISIBLE : View.GONE);
		}
	}
...
//				}
	...
	private void setNotificationAsRead(Notification notification) {
		...
		HLRequestTracker.getInstance((LUISSApp) activity.getApplication()).handleCallResult(this, activity, result);
	}

	public void updateNotifications() {
		if (mListener != null && Utils.isStringValid(mListener.getUserId()) && LUISSApp.notificationsFragmentVisible/* && mListener.getProfileType() == ProfileHelper.ProfileType.ME*/) {
