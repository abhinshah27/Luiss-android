...
import tech.icrossing.lsm.activities_and_fragments.SelectIdentityActivity;
import tech.icrossing.lsm.activities_and_fragments.WebViewActivity;
import tech.icrossing.lsm.activities_and_fragments.WebViewActivityDocuments;
...
import tech.icrossing.lsm.services.FCMNotNeededException;
...
public class Utils {
	...
	public static void applyRegularFontToMenuItem(Context context, MenuItem mi) {
		applyFontToMenuItem(context, mi, R.string.osReg);
	}
...
//								LogUtils.i(context.getClass().getCanonicalName(), "Refreshed FCM token: " + refreshedToken);
								...
								if (!LUISSApp.fcmTokenSent || (Utils.isStringValid(refreshedToken) && !fcm.equals(refreshedToken))) {
	...
	public static void logOut(Activity activity, @Nullable MaterialDialog dialog) {
		...
		Realm.deleteRealm(LUISSApp.realmConfig);
